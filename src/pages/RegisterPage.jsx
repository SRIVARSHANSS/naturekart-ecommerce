    ...
    <div className="min-h-screen bg-gradient-to-br from-green-50 via-white to-emerald-50 flex items-center justify-center p-4">
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        {['🌿','🍃','🌱','🌾','✨','🌻'].map((e, i) => (
          <motion.div key={i} className="absolute text-4xl opacity-10"
            style={{ left: `${5 + i * 17}%`, top: `${10 + (i % 3) * 30}%` }}
      ...
      <motion.div initial={{ opacity: 0, y: 32, scale: 0.97 }} animate={{ opacity: 1, y: 0, scale: 1 }}
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="mt-4 text-3xl font-extrabold text-stone-800">Create account</h1>
          <p className="text-stone-400 mt-1 text-sm">Join NatureKart — it's free</p>
        </div>
        ...
        <div className="bg-white/90 backdrop-blur-xl rounded-3xl shadow-2xl shadow-stone-200/60 border border-white p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <FloatInput id="name"     label="Full Name"        value={name}     onChange={e => setName(e.target.value)}     error={errors.name} />
            <FloatInput id="email"    label="Email address"    type="email"   value={email}    onChange={e => setEmail(e.target.value)}    error={errors.email} />
            <FloatInput id="password" label="Password"         type="password" value={password} onChange={e => setPassword(e.target.value)} error={errors.password} />
            <FloatInput id="confirm"  label="Confirm Password" type="password" value={confirm}  onChange={e => setConfirm(e.target.value)}  error={errors.confirm} />
            ...
            <motion.button type="submit" whileHover={{ scale: 1.02, boxShadow: '0 20px 40px rgba(16,185,129,0.3)' }}
              ...
              className="w-full py-4 mt-2 bg-gradient-to-r from-green-500 to-emerald-500 text-white font-extrabold text-base rounded-2xl shadow-lg shadow-green-200/60 flex items-center justify-center gap-2 disabled:opacity-70">
              {loading ? (
                <><motion.span animate={{ rotate: 360 }} transition={{ repeat: Infinity, duration: 0.8, ease: 'linear' }}
                  className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full inline-block" /> Creating…</>
              ) : 'Create Account →'}
            </motion.button>
          </form>
